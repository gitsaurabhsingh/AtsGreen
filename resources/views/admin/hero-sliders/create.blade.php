...
@section('header')
    <div class="flex items-center space-x-3">
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Create Banner</h2>
    </div>
@endsection
...
<div class="max-w-4xl">
    <form action="{{ route('admin.hero-sliders.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-md shadow-sm border border-gray-100 p-6 space-y-6">
        @csrf
        
        <div>
            <label class="block text-sm font-bold text-gray-800 mb-2">Banner Image</label>
            <input type="file" name="image" id="image" required class="block w-full text-sm text-gray-500 border border-gray-300 rounded-md
                file:mr-4 file:py-2 file:px-4
                file:border-0 file:border-r file:border-gray-300
                file:text-sm file:font-semibold
                file:bg-gray-50 file:text-gray-700
                hover:file:bg-gray-100 cursor-pointer" accept="image/*">
            <p class="mt-1 text-xs text-gray-500">Required. Recommended size: 1920x1080px.</p>
        ...
        <div>
            ...
            <input type="url" name="target_url" id="target_url" value="{{ old('target_url') }}" class="focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3" placeholder="https://">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="datetime-local" name="start_date" id="start_date" value="{{ old('start_date') }}" class="focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3">
            ...
            <div>
                ...
                <input type="datetime-local" name="end_date" id="end_date" value="{{ old('end_date') }}" class="focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3">
