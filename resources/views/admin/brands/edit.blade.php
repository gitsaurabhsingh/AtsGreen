...
@section('header', 'Edit Brand: ' . $brand->name)
...
<div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 max-w-2xl">
    <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('name', $brand->name) }}">
            @error('name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
        </div>

        <div class="mb-4">
            <label for="slug" class="block text-sm font-medium text-gray-700 mb-1">Slug (Optional)</label>
            <input type="text" name="slug" id="slug" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('slug', $brand->slug) }}">
        ...
        <div class="mb-4">
            ...
            @if($brand->banner_image)
                <div class="mb-2">
                    <img src="{{ $brand->banner_image }}" alt="Banner" class="h-24 w-auto object-cover rounded shadow">
                </div>
            @endif
            ...
            <small class="text-gray-500">Leave empty to keep existing banner.</small>
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" rows="4" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border">{{ old('description', $brand->description) }}</textarea>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2 bg-brand text-white rounded-md hover:bg-brand-dark transition">Update Brand</button>
        </div>
    </form>
</div>
@endsection
