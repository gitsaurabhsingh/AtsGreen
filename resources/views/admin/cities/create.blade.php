    ...
    <form action="{{ route('admin.cities.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="flex items-center mt-6">
                <input type="checkbox" name="status" id="status" value="1" {{ old('status') ? 'checked' : '' }} class="h-4 w-4 text-brand focus:ring-brand border-gray-300 rounded">
        ...
        <div class="flex justify-end gap-3 mt-8 border-t pt-4">
            ...
            <button type="submit" class="px-4 py-2 bg-brand text-white rounded-md hover:bg-brand-dark transition">Save City</button>
        </div>
    </form>
</div>
@endsection
