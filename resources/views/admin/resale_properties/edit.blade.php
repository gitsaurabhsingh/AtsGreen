...
@section('header', 'Edit Resale Property')
...
<div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 max-w-4xl">
    ...
    <form action="{{ route('admin.resale-properties.update', $resaleProperty) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="md:col-span-2">
                ...
                <select name="project_id" id="project_id" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border">
                    ...
                    @foreach($projects as $p)
                        <option value="{{ $p->id }}" {{ old('project_id', $resaleProperty->project_id) == $p->id ? 'selected' : '' }}>{{ $p->project_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <select name="resale_category_id" id="resale_category_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border">
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ old('resale_category_id', $resaleProperty->resale_category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="title" id="title" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('title', $resaleProperty->title) }}">
            ...
            <div>
                ...
                <input type="text" name="status" id="status" placeholder="e.g. New Launch, Ready to Move" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('status', $resaleProperty->status) }}">
            ...
            <div>
                ...
                <input type="text" name="locality" id="locality" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('locality', $resaleProperty->locality) }}">
            ...
            <div>
                ...
                <input type="text" name="rera_number" id="rera_number" placeholder="e.g. UPRERAPRJ12345" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('rera_number', $resaleProperty->rera_number) }}">
            ...
            <div>
                ...
                @if($resaleProperty->rera_qr)
                    <div class="mt-2 text-xs text-green-600">Current: {{ basename($resaleProperty->rera_qr) }}</div>
                @endif
            ...
            <div>
                ...
                <input type="text" name="price_label" id="price_label" placeholder="e.g. ₹ 2.5 Cr*" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('price_label', $resaleProperty->price_label) }}">
                @error('price_label') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            </div>

            <div class="flex items-center mt-6">
                <input type="checkbox" name="featured" id="featured" value="1" {{ old('featured', $resaleProperty->featured) ? 'checked' : '' }} class="h-4 w-4 text-brand focus:ring-brand border-gray-300 rounded">
        ...
        <div class="border-t border-gray-100 pt-6 mt-6 mb-6">
            ...
            <div class="mb-6">
                ...
                <textarea name="short_description" id="short_description" rows="2" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border">{{ old('short_description', $resaleProperty->short_description) }}</textarea>
            </div>

            <div class="mb-6">
                <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Full Description</label>
                <textarea name="description" id="description" rows="5" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border">{{ old('description', $resaleProperty->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="price" id="price" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('price', $resaleProperty->price) }}">
                </div>
                <div>
                    <label for="area" class="block text-sm font-medium text-gray-700 mb-1">Area</label>
                    <input type="text" name="area" id="area" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('area', $resaleProperty->area) }}">
                </div>
                <div>
                    <label for="bedrooms" class="block text-sm font-medium text-gray-700 mb-1">Bedrooms</label>
                    <input type="text" name="bedrooms" id="bedrooms" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('bedrooms', $resaleProperty->bedrooms) }}">
                </div>
                <div>
                    <label for="bathrooms" class="block text-sm font-medium text-gray-700 mb-1">Bathrooms</label>
                    <input type="text" name="bathrooms" id="bathrooms" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('bathrooms', $resaleProperty->bathrooms) }}">
                </div>
                <div>
                    <label for="contact_name" class="block text-sm font-medium text-gray-700 mb-1">Contact Name</label>
                    <input type="text" name="contact_name" id="contact_name" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('contact_name', $resaleProperty->contact_name) }}">
                </div>
                <div>
                    <label for="contact_phone" class="block text-sm font-medium text-gray-700 mb-1">Contact Phone</label>
                    <input type="text" name="contact_phone" id="contact_phone" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" value="{{ old('contact_phone', $resaleProperty->contact_phone) }}">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    @if($resaleProperty->featured_image)
                        <div class="mt-2 text-xs text-green-600">Current: {{ basename($resaleProperty->featured_image) }}</div>
                    @endif
                </div>
                <div>
                    ...
                    @if($resaleProperty->location_map_image)
                        <div class="mt-2 text-xs text-green-600">Current: {{ basename($resaleProperty->location_map_image) }}</div>
                    @endif
                </div>
                <div>
                    ...
                    @if($resaleProperty->site_plan_image)
                        <div class="mt-2 text-xs text-green-600">Current: {{ basename($resaleProperty->site_plan_image) }}</div>
                    @endif
                </div>
                <div>
                    ...
                    @if($resaleProperty->payment_plan_image)
                        <div class="mt-2 text-xs text-green-600">Current: {{ basename($resaleProperty->payment_plan_image) }}</div>
                    @endif
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="payment_plan_text" id="payment_plan_text" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring focus:ring-brand focus:ring-opacity-50 px-3 py-2 border" placeholder="Alternatively, describe the payment plan here...">{{ old('payment_plan_text', $resaleProperty->payment_plan_text) }}</textarea>
            </div>
        </div>
        ...
        <div class="border-t border-gray-100 pt-6 mt-6 mb-6" x-data="{ faqs: {{ Js::from(old('faqs', $resaleProperty->faqs->map(function($f) { return ['id' => $f->id, 'question' => $f->question, 'answer' => $f->answer]; })->toArray())) }} }">
